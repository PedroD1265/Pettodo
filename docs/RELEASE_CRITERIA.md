...
- animal, case, and related data survive normal user return/re-entry
- the system behaves as a real web product, not only a local demo

### Must not be true at release
**[confirmed]**
- core records are still local-only
- the app cannot function as a real shared system
- critical product truth depends on seeded demo state only
...
### Why
**[confirmed]**
Current known non-real areas still include:
- real auth
- real persistence
...
The beta cut line still requires real system implementation.
