...
**[confirmed]** PETTODO is currently a **very advanced frontend prototype / demo-functional web app** with substantial product breadth, but it is not yet a fully real multi-user production system.

It already expresses strong product structure, but still depends heavily on local/demo logic in critical areas.
...
- backend / API
- real database
- real multi-user data model in production
- real authentication
...
### Current known demo/local limitations
**[confirmed]**
- app data is local-first and local-only
- persistence depends heavily on localStorage
- OTP has been simulated
- matching has been heuristic, not a real advanced identity system
- some flows were built for demo readiness rather than infrastructure readiness
